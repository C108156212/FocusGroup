...
while True:
    ...
    for request_n in request_file.index:
        ...
        try:
            ...
            comment_root = "\\Dcard_API_PostData\\" + \
                str(request_file["id"][request_n]) + ".json"
            comment_path = "{}{}".format(
                pathlib.Path(__file__).parent, comment_root)
            ...
            for comment_n in comment_file.index:
                ...
                try:
                    ...
                    degree = float(TCSA.Degree(content))
                    Orientation = str(TCSA.Orientation(content))
                    data = {"id": id, "postId": postId,
                            "updatedAt": updatedAt, "content": content, "degree": degree, "orientation": Orientation}
